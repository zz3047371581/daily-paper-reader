...
def main() -> None:
    parser = argparse.ArgumentParser(description="抓取 CVPR 论文并同步到 Supabase。")
    ...
    if raw_path:
        ...
        token = f"cvpr-papers-{int(args.year_end) - int(args.year_count) + 1}-{int(args.year_end)}"
    ...
    if not args.skip_fetch:
        ...
        run_step("Step 1 - fetch CVPR papers", fetch_cmd)
    else:
    ...
    run_step("Step 2 - sync CVPR papers to Supabase", sync_cmd)
