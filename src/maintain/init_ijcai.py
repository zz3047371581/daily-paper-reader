...
def main() -> None:
    parser = argparse.ArgumentParser(description="抓取 IJCAI 论文并同步到 Supabase。")
    ...
    if raw_path:
        ...
        token = f"ijcai-papers-{int(args.year_end) - int(args.year_count) + 1}-{int(args.year_end)}"
    ...
    if not args.skip_fetch:
        ...
        fetch_cmd = [
            python,
            os.path.join(SCRIPT_DIR, "fetchers", "fetch_ijcai.py"),
        ...
        run_step("Step 1 - fetch IJCAI papers", fetch_cmd)
    else:
    ...
    run_step("Step 2 - sync IJCAI papers to Supabase", sync_cmd)
