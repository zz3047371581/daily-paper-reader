...
def main() -> None:
    parser = argparse.ArgumentParser(description="维护入口：IJCAI 抓取 + Supabase 同步。")
    ...
    cleanup_backend(backend_key="ijcai", retention_days=args.retention_days, skip_cleanup=args.skip_cleanup)

    start_year = int(args.year_end) - int(args.year_count) + 1
    raw_path = str(args.raw_input or "").strip() or default_raw_path(
        f"ijcai-papers-{start_year}-{int(args.year_end)}",
        run_date,
    )
    ...
    init_cmd = [
        sys.executable,
        os.path.join(os.path.dirname(__file__), "init_ijcai.py"),
        "--year-end",
    ...
    run_step("Maintain IJCAI", init_cmd)
